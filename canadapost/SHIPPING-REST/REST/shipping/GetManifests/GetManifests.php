...
// REST URL
$service_url = 'https://ct.soa-gw.canadapost.ca/rs/' . $mailedBy . '/' . $mobo . '/manifest?start=20010101';
...
if (!$xml) {
	...
	if ($xml->{'manifests'} ) {
		$manifests = $xml->{'manifests'}->children('http://www.canadapost.ca/ws/manifest-v4');
		if ( $manifests->{'link'} ) {
			foreach ( $manifests->{'link'} as $link ) {  
