...
// REST URL
$service_url = 'https://ct.soa-gw.canadapost.ca/rs/customer/' . $mailedBy;
...
if (!$xml) {
	...
	if ($xml->{'customer'} ) {
		$customer = $xml->{'customer'}->children('http://www.canadapost.ca/ws/customer');
		if ( $customer->{'customer-number'} ) {
			...
			echo "\n";
			if ( $customer->{'links'} ) {	
				foreach ( $customer->{'links'}->{'link'} as $link ) {  
